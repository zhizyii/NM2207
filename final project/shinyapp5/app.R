...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("year", "Select Year", choices = 2010:2021)
    ),
    mainPanel(
...
server <- function(input, output) {
  filtered_data <- reactive({
    new_data %>%
      ...
      select(Industry, input$year)
  })
  
  output$emissions_plot <- renderPlot({
    ggplot(filtered_data(), aes(x = Industry, y = get(input$year), fill = Industry)) +
      geom_bar(stat = "identity") +
      labs(x = "Industry", y = paste(input$gas_type, "Emissions"), fill = "Industry") +
      ggtitle(paste(input$gas_type, "Emissions for", input$year)) +
